...
def test_tavern_adapters_and_card_ids_are_deterministic():
    ...
    assert (
        tavern.resolve_card_url(page)
        == "https://cards.character-tavern.com/creator/example-card.png"
    )
    assert (
        tavern.card_id_from_url("https://host.invalid/cards/A card.charx") == "A_card"
    )
...
def test_spicychat_url_parser_accepts_supported_routes_only():
    ...
    assert (
        spicychat.parse_character_id(f"https://spicychat.ai/chatbot/{character_id}")
        == character_id
    )
    assert (
        spicychat.parse_character_id(f"https://spicychat.ai/characters/{character_id}")
        == character_id
    )
